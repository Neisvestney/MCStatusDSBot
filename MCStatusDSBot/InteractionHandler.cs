...
internal class InteractionHandler : DiscordClientService
{
    ...
    {
        using (IServiceScope scope = _provider.CreateScope()) // Creating scope to inject scoped db context
        {
            ...
            await _interactionService.AddModulesAsync(Assembly.GetEntryAssembly(), scope.ServiceProvider); // scoped provider
